...
export function parseBeastXML(text) {
  ...
  function addNode(el, id) {
    ...
    const n = {
      ...
      machinery: MACHINERY_TAGS.has(tag) || STATISTIC_TAGS.has(tag) && isLoggedOnly(el, doc),
  ...
  for (const [id, n] of nodes) {
    if (!n.el || n.isHyper) continue;
    for (const d of n.el.getElementsByTagName('*')) {
      ...
      if (nodes.get(child).type === 'clamped' && SUBST_TAGS.has(n.tag)) continue;
      addEdge(child, id, n.type === 'deterministic' ? 'dashed' : 'solid');
    }
  }
...
/** true if this statistic is only ever referenced from a <log> block. */
function isLoggedOnly(el, doc) {
  const id = el.getAttribute('id');
  if (!id) return false;
  let inLog = 0, outLog = 0;
  for (const r of doc.getElementsByTagName('*')) {
    if (r.getAttribute('idref') !== id) continue;
    let p = r.parentElement, isLog = false;
    while (p) { if (p.tagName === 'log' || p.tagName === 'logTree') { isLog = true; break; } p = p.parentElement; }
    isLog ? inLog++ : outLog++;
  }
  return outLog === 0;
}
